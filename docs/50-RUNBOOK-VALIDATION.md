...
|---|---|---|---|---|---|
| `<machine-de-test>` | <adresse locale> | Debian 12 · **YunoHost 12.1.26** | 2 c / 3,7 Go | 300 Go | **Cible des gates G3** |
| `<machine-hote>` | <adresse locale> | Debian 12 · YunoHost + Forgejo + Docker | 16 c / 31 Go | 220 Go | Forge, et hôte de la VM pour G4 |
...
par YunoHost** sur `<machine-de-test>` comme sur `<machine-hote>`. Docker sur `<machine-hote>` entre en conflit supplémentaire avec
le réseau Incus.

Installer Incus sur `<machine-de-test>` casserait l'environnement de test YunoHost. On sépare donc :

- **G3 sur `<machine-de-test>`, sans Incus** — l'instance est déjà exactement la cible (bookworm, amd64,
  YunoHost 12.1.26, format v2, helpers 2.1). Rien à installer.
- **G4 dans une VM isolée sur `<machine-hote>`** — seule façon propre d'avoir Incus sans toucher à l'hôte.

## Étage 1 — G3 sur `<machine-de-test>`
...
### Cycle exécuté par `yunopack test --host=<machine-de-test>`

```bash
rsync -a ./<app>_ynh/ la machine de test:/tmp/yunopack/<app>_ynh/

ssh la machine de test yunohost app install /tmp/yunopack/<app>_ynh \
      --debug --force \
      -a "domain=test.local&path=/<app>&init_main_permission=visitors"

ssh la machine de test curl -sI https://test.local/<app>/          # 200 ou 30x attendu
ssh la machine de test yunohost backup create --apps <app>
ssh la machine de test yunohost backup restore <archive> --apps <app> --force
ssh la machine de test yunohost app remove <app>
```
...
```bash
ssh la machine de test yunohost app remove <app> --purge
```
...
## Étage 2 — G4 sur `<machine-hote>` *(optionnel)*

Requis uniquement pour une contribution au catalogue officiel, qui publie un niveau de qualité 0-8.

### Principe

Une VM Debian sous QEMU/libvirt sur `<machine-hote>`, avec Incus **à l'intérieur**. L'isolation résout d'un
...
On n'installe **rien** sur `<machine-hote>` lui-même : c'est une machine de production qui héberge Forgejo.
